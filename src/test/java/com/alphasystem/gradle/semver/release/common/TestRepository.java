...
public class TestRepository {
  ...
  public TestRepository(Repository repository) {
    ...
    this.workingDirectory =
        Paths.get(repository.getDirectory().getParentFile().getAbsolutePath()).toFile();
  }
  ...
  public TestRepository checkoutTag(String branchName, String tagName) {
    try {
      var ref =
          git.checkout().setCreateBranch(true).setName(branchName).setStartPoint(tagName).call();

      logger.trace("Checkout tag: {}, current branch: {}", ref.getName(), getBranchName());
    } catch (Exception e) {
  ...
  public RevTag isHeadTagAnnotated() {
    try {
      return (RevTag) new RevWalk(repository).parseAny(repository.resolve(getHeadTag()));
    } catch (IOException e) {
