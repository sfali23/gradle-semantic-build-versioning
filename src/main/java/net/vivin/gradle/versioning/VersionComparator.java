...
public class VersionComparator implements Comparator<String> {
  private static final Pattern VERSION_AT_START_PATTERN =
      Pattern.compile("^(\\d++\\.\\d++\\.\\d++)");
  private static final Pattern PRE_RELEASE_PART_PATTERN =
      Pattern.compile("(?<=^\\d+\\.\\d+\\.\\d+)-(?<preReleasePart>.*)$");
  ...
  public int compare(String a, String b) {
    ...
    if (!aVersionComponents[VersionComponent.MAJOR.getIndex()].equals(
        bVersionComponents[VersionComponent.MAJOR.getIndex()])) {
    ...
    } else if (!aVersionComponents[VersionComponent.MINOR.getIndex()].equals(
        bVersionComponents[VersionComponent.MINOR.getIndex()])) {
    ...
    } else if (!aVersionComponents[VersionComponent.PATCH.getIndex()].equals(
        bVersionComponents[VersionComponent.PATCH.getIndex()])) {
      ...
      for (;
          (i < aPreReleaseComponents.length) && (i < bPreReleaseComponents.length) && matching;
          i++) {
      ...
      if (matching) {
        ...
        boolean aNumericComponent =
            NUMERIC_COMPONENT_PATTERN.matcher(aNonMatchingComponent).matches();
        boolean bNumericComponent =
            NUMERIC_COMPONENT_PATTERN.matcher(bNonMatchingComponent).matches();
